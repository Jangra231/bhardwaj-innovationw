...
export function generateStaticParams() {
  return PRODUCTS_DATA.filter((p) => p.category === "Hardware").map((p) => ({ id: p.id }));
}

export function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  return params.then((p) => {
    const product = PRODUCTS_DATA.find((pr) => pr.id === p.id && pr.category === "Hardware");
...
export default async function ProductDetailPage({
  ...
  const product = PRODUCTS_DATA.find((p) => p.id === id && p.category === "Hardware");
